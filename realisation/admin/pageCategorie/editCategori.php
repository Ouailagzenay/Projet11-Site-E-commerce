<?php
    ...
    if(isset($_GET['id'])){
        $id = $_GET['id'];
        $categorie = $ecommerceManager->getCategorie($id);

    }

    if(isset($_POST['update'])){

		$categorieToEdit = new Ecommerce();
	
		$categorieToEdit->setNomCategory($_POST['Nom']);

        $ecommerceManager->editCategorie($categorieToEdit, $id);
...
<div class="col py-3 container-fluid ">
    ...
    <input type="text" class="form-control" value=<?php echo $categorie->getNomCategory() ?> name="Nom"  required>

  </div>
	    <button type="submit" name="update" class="btn btn-primary">Modiffer</button>
	</div>
	</div>
    </form>
